...
void LSMLiquid::onMouseMove(const Vector2D pos) {
    mPrevMousePos = mMousePos;
    mMousePos = pos;
}

void LSMLiquid::onMouseButtonPress(int button, int actions, int mods) {
    mMouseButtons[button] = actions == GLFW_PRESS;
}

void LSMLiquid::onKeyPress(int key, int action, int mods) {
...
void LSMLiquid::onFramebufferResize(const u32 width, const u32 height) {
    glViewport(0, 0, static_cast<GLsizei>(width), static_cast<GLsizei>(height));
}

void LSMLiquid::saveCurrentFrame() const {
    if (mTexData.empty()) {
